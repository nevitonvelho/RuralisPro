  ...
  return (
    <div className="mt-4 mb-6 border border-slate-300 rounded-lg overflow-hidden avoid-break shadow-none break-inside-avoid print:rounded-none print:border-collapse print:border-slate-300">
      ...
      <table className="w-full text-sm text-left print:border print:border-slate-300">
        <thead>
          <tr className="bg-slate-100 border-b border-slate-300 text-xs uppercase text-slate-500 print:bg-slate-100 print:text-black print:font-bold print:border-slate-300">
            {Object.keys(rows[0] || {}).map((header, i) => (
              <th key={i} className="p-2 font-bold print:text-black print:border-r print:border-slate-300">{header}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, index) => (
            <tr key={index} className="border-b border-slate-300 last:border-0 bg-white print:border print:border-slate-300">
              {Object.values(row).map((val: any, i) => (
                <td key={i} className="p-2 text-black font-medium print:text-black print:border-r print:border-slate-300">{val}</td>
              ))}
            </tr>
          ))}
        </tbody>
