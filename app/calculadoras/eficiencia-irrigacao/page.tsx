...
export default function EficienciaIrrigacaoPage() {
  ...
  // INPUTS - TESTE DE COLETORES (Simplificado)
  const [mediaGeral, setMediaGeral] = useState<number | string>(""); // mm (Lâmina média coletada)
  const [mediaInferior, setMediaInferior] = useState<number | string>(""); // mm (Média do 1/4 menor - 25% mais secos)
  ...
  const resultados = useMemo(() => {
    ...
    let du = (mi / mg) * 100;
    if (du > 100) du = 100; // Proteção contra input errado
    ...
    // Se DU é 80%, para aplicar 10mm líquidos, preciso aplicar 10 / 0.8 = 12.5mm brutos
    ...
    // Dinheiro gasto bombeando água que não contribui para a uniformidade efetiva
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
           ...
           {/* Seção 1: Dados do Teste (Coletores) */}
                 ...
                 <div className={`space-y-6 transition-all duration-500 ${!isAuthenticated ? 'opacity-40 pointer-events-none filter blur-sm select-none' : ''}`}>
                    ...
                    <div className="print-card bg-slate-900 text-white rounded-xl p-8 shadow-xl relative overflow-hidden">
                        ...
                        <div className={`absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl -mr-20 -mt-20 pointer-events-none ${
                            resultados.status === 'excelente' ? 'bg-emerald-500/20' : 
                        ...
                        <div className="relative z-10">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className={`px-2 py-1 rounded text-[10px] font-bold uppercase border ${
                                     resultados.status === 'excelente' ? 'bg-emerald-500/20 border-emerald-500 text-emerald-300' : 
                            ...
                            <div className="w-full bg-slate-800 rounded-full h-2 mb-2 overflow-hidden">
                                <div 
                                    className={`h-full transition-all duration-1000 ${
                                        resultados.status === 'excelente' ? 'bg-emerald-500' : 
