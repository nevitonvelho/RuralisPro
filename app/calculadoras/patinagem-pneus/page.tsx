...
export default function PatinagemPneusPage() {
  ...
  // O método mais preciso é marcar uma distância (ex: 50m ou 100m)
  // Contar voltas com carga (trabalhando) vs voltas vazio (levantado ou estrada)
  ...
  const resultados = useMemo(() => {
    ...
    if (vCarga === 0) return { patinagem: 0, status: 'neutral', desperdicioL: 0, desperdicioR: 0 };

    // 1. Cálculo de Patinagem (%)
    // Fórmula: ((VoltasCarga - VoltasVazio) / VoltasCarga) * 100
    // Ex: Precisei de 12 voltas pra cobrir 50m trabalhando, mas só 10 voltas no vazio.
    // (12 - 10) / 12 = 0.166 = 16.6% Patinagem
    const patinagem = ((vCarga - vVazio) / vCarga) * 100;

    // 2. Classificação (Benchmark para 4x4 / TDA)
    let status = 'ideal'; // 8 a 12%
    if (patinagem < 6) status = 'muito_pesado'; // Risco de compactação / Power hop
    if (patinagem > 15) status = 'muito_leve'; // Perda de tração / Gasto pneu

    // 3. Estimativa de Desperdício (Se acima do ideal)
    // Se a patinagem é 20% e o ideal é 10%, estou perdendo 10% de eficiência teórica
    let desperdicioL = 0;
    let desperdicioR = 0;

    if (patinagem > 12) {
        const excesso = (patinagem - 12) / 100;
        // Estimativa simplificada: Excesso de patinagem % * Consumo
        desperdicioL = l_h * excesso;
        desperdicioR = desperdicioL * p_diesel;
    }

    return {
        patinagem: patinagem < 0 ? 0 : patinagem, // Evitar negativo se usuário inverter
        status,
        desperdicioL,
        desperdicioR
    };
  ...
  const shareText = `🚜 *Diagnóstico de Tração*\n\n🔄 *Patinagem:* ${fmtNum(resultados.patinagem)}%\n📊 *Status:* ${resultados.status === 'ideal' ? '✅ Ideal' : resultados.status === 'muito_pesado' ? '⚠️ Muito Pesado (Retire Lastro)' : '🛑 Patinando (Adicione Lastro)'}\n\n${resultados.desperdicioR > 0 ? `💸 *Perda Estimada:* ${fmtMoeda(resultados.desperdicioR)}/hora` : ''}`;

  return (
    <CalculatorLayout
      title="Patinagem de Pneus"
      subtitle="Ajuste fino de lastragem para máxima tração e economia de combustível."
      category="Maquinário"
           ...
           <section className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              <div className="flex items-center gap-3 mb-6 border-b border-slate-100 pb-4">
                 ...
                 <div>
                    ...
                    <p className="text-xs text-slate-400">Marque uma distância fixa (Ex: 50m) e conte as voltas da roda</p>
                 </div>
              </div>
              ...
              <div className="mt-4 p-3 bg-indigo-50 rounded-lg border border-indigo-100 flex gap-2 items-start text-xs text-indigo-800">
                  ...
                  <p>
                    <strong>Como medir:</strong> Conte quantas voltas o pneu dá para percorrer 100m no asfalto/terra dura (Vazio). Depois, conte quantas voltas ele dá para percorrer os mesmos 100m operando com o implemento enterrado (Carga).
                  </p>
              </div>
           </section>
                    ...
                    {resultados.status === 'muito_leve' && resultados.desperdicioR > 0 && (
                        <div className="print-card bg-rose-50 border border-rose-200 rounded-xl p-6 shadow-lg animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <p className="text-xs text-rose-800 mb-4 leading-relaxed">
                                Você está gastando combustível para girar o pneu sem sair do lugar.
                            </p>
