...
export default function DeplecaoAguaPage() {
  ...
  const resultados = useMemo(() => {
    ...
    // Fórmula: (CC - PMP) * Ds * Z / 10
    const cta = (_cc - _pmp) * _ds * _z / 10;

    // 2. Armazenamento Atual em mm (Acima do PMP)
    // Se a umidade atual for menor que PMP, é zero (planta morta/estresse severo)
    let atualMm = (_uAtual - _pmp) * _ds * _z / 10;
    if (atualMm < 0) atualMm = 0;
    if (atualMm > cta) atualMm = cta; // Não considera saturação acima da CC para cálculo de disponibilidade útil

    // 3. Lâmina de Irrigação (O quanto falta para encher o tanque até a CC)
    // Fórmula direta: (CC - U_atual) * Ds * Z / 10
    let irrigacaoNecessaria = (_cc - _uAtual) * _ds * _z / 10;
    if (irrigacaoNecessaria < 0) irrigacaoNecessaria = 0; // Solo saturado
        ...
        <div className="lg:col-span-5 relative">
            <div className="sticky top-10 print:static">
                 ...
                 <div className={`space-y-6 transition-all duration-500 ${!isAuthenticated ? 'opacity-40 pointer-events-none filter blur-sm select-none' : ''}`}>
                    ...
                    {/* Card Principal: LÂMINA DE REPOSIÇÃO */}
                    ...
                    {/* Card Secundário: O TANQUE DE ÁGUA (Visualização Gráfica) */}
