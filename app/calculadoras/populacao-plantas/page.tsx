...
export default function PopulacaoPage() {
  ...
  // INPUTS
  const [entrelinha, setEntrelinha] = useState<number | string>(""); // cm
  const [sementesMetro, setSementesMetro] = useState<number | string>(""); // qtd
  ...
  const resultados = useMemo(() => {
    ...
    if (eCm === 0) return { metrosLineares: 0, sementesHa: 0, plantasHa: 0, perda: 0 };

    // 1. Converter espaçamento para metros
    const eMetro = eCm / 100;

    // 2. Metros lineares em 1 hectare (10.000m²)
    // Se eu tenho linhas a cada 0.5m, em 10.000m² tenho quantos metros de linha?
    const metrosLineares = 10000 / eMetro;

    // 3. Total de Sementes (População Inicial)
    const sementesHa = metrosLineares * sMetro;

    // 4. Stand Final (Considerando Germinação)
        ...
        <div className="lg:col-span-7 space-y-6">
           ...
           <section className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                 <InputGroup 
                    label="Espaçamento Entrelinhas (cm)" 
                    icon={<Ruler size={16}/>} 
                    value={entrelinha} 
                    onChange={setEntrelinha} 
                 ...
                 <div className={`space-y-6 transition-all duration-500 ${!isAuthenticated ? 'opacity-40 pointer-events-none filter blur-sm select-none' : ''}`}>
                    ...
                    <div className="print-card bg-slate-900 text-white rounded-xl p-8 shadow-xl relative overflow-hidden">
                        <div className="relative z-10">
                            <p className="text-emerald-400 font-bold uppercase tracking-wider text-xs mb-1">
                                Stand Final Estimado
                            </p>
                            <p className="text-slate-400 text-[10px] mb-6">
                                Plantas viáveis por hectare
                            </p>
                    ...
                    <div className="print-card bg-white border border-slate-200 rounded-xl p-6 shadow-lg">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm font-bold text-slate-400 bg-slate-100 px-3 py-1 rounded-full">metros/ha</span>
                        </div>
                        <p className="text-[10px] text-slate-400 mt-3 leading-tight">
                            Use este valor para calcular compra de sementes (kg), adubo ou fita de gotejamento.
                        </p>
                    </div>

                    {/* Alerta de Densidade */}
                    <div className="bg-amber-50 border border-amber-100 p-4 rounded-lg flex gap-3 items-start no-print">
                        <AlertTriangle size={18} className="text-amber-600 shrink-0 mt-0.5" />
                        <div className="text-xs text-amber-800">
                            <span className="font-bold block mb-1">Dica de Manejo:</span>
                            Aumentar a população acima do recomendado para o híbrido/cultivar pode aumentar a competição e reduzir a produtividade, além de facilitar doenças.
                        </div>
                    </div>
